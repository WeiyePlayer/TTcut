...
- Windows ball-detection analysis now defaults to temporal stride 3 while preserving per-frame timestamps and using constrained interpolation for intermediate positions. One frozen-video run reduced total time by about 25.3%, but its results differed from the frame-by-frame baseline, so recognition-quality equivalence or the same speedup on macOS is not claimed.
