...
> The current stable Windows release is `v1.3.6`; the current stable macOS 15+ Apple Silicon release is `v1.3.4`.
...
- Restored bounce-count multi-selection and shared-boundary timeline interaction, and added a current-rally editing target, 8-pixel playhead snapping, and `A`/`D` boundary editing.
