...
> 当前稳定版本为 `v1.1.3`，面向 Windows x64；应用不再按 Windows build 或系统版本进行硬拦截。

## 下载与安装

1. 从 [TTcut v1.1.3 Release](https://github.com/WeiyePlayer/TTcut/releases/tag/v1.1.3) 下载 `TTcut-1.1.3-x64-Setup.exe`。
...
- 将默认弹跳去重间隔调整为 0.315 秒，并始终与上一个保留事件比较，减少短时间重复候选造成的回合板数虚高。
- 提高分析和标定 Worker 对 GoPro HEVC、多音轨、时间码及遥测等复杂 MP4 流布局的解码兼容性。
- 视频解码明显提前结束时返回明确错误，不再把不完整视频误报为“未识别到有效回合”。

详见 [v1.1.3 发布说明](docs/release-notes-v1.1.3.md)。
