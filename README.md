...
视频、分析结果和历史记录只保存在本机；软件不要求登录、不上传视频、不采集遥测。Windows 完整安装包与 macOS 安装包已包含运行资源，安装完成后可以离线分析、预览和剪辑。

> 当前 Windows 稳定版本为 `v1.3.6`；macOS 15+ Apple Silicon 当前稳定版本为 `v1.3.4`。

## 下载与安装

1. Windows x64 从 [TTcut Release](https://github.com/WeiyePlayer/TTcut/releases) 下载完整安装包。
...
4. Windows 完整安装包已包含分析模型、运行时和视频处理工具，安装后无需再下载或导入组件。

## v1.3.6 更新

- Windows 完整安装包改为内置分析模型、硬件加速/CPU 运行时和视频处理工具，取消模型与媒体组件的下载、导入和安装流程。
- 恢复按板数多选与相邻边界时间线交互，并新增当前回合目标、8 像素播放头吸附及 `A`/`D` 边界编辑。
- 修复自动标定跳帧、混合回合重叠、HEVC 时长边界和视频色彩范围输出问题。

详见 [v1.3.6 发布说明](docs/release-notes-v1.3.6.md)。
...
- 按任意顺序点击球桌四角；标满四点后可拖动编号点修正位置。
- 确认四点没有重合、越界，并形成足够大的凸四边形后，点击“开始分析”。
...
- **所有回合**：剪辑全部有效回合。
- **精彩回合**：使用“落台判定”时按板数筛选，使用“连续运动”时按时长档位筛选。
...
完整安装包内置运行所需的球识别、球桌识别和视频处理资源。本地分析负责：

- 自动或手动球桌标定与坐标映射。
- 定位乒乓球，并按“连续运动”或“落台判定”整理有效回合。
- 使用统一的球台区域、时间间隔和回合规则整理有效回合。
- 根据本机环境选择 GPU 加速或 CPU 处理。

运行资源随应用一次安装完成，并在打包、启动和任务执行前按固定大小与 SHA-256 校验；资源损坏时会明确提示重新安装。
...
视频处理工具随完整安装包提供，无需额外安装扩展。程序不会放大低分辨率视频。
...
- 板数是落点事件的代理值，不是真实击球计数；“连续运动”的精彩回合仍按时长档位筛选，自定义剪辑仅在新分析结果包含板数元数据时显示和筛选板数。
