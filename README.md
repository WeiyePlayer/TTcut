...
- 设置新增“分析精度”：默认模式保持原有整段分析；高精模式会先筛选候选回合，再在候选区间内进行更细致的二次分析，耗时会增加。
- 对可变帧率视频，标定后会优先生成固定帧率处理媒体，再用于分析、预览、剪辑和新建 XML；原始视频仍用于身份、标定和默认输出命名。
- 固定帧率处理所需空间不足、转码失败或输出校验失败时，应用会明确提示并回退使用原始可变帧率视频继续分析。
- 优化回合识别流程与设置默认值，并修复相关进度、历史记录和导出衔接问题。

详见 [v1.2.9 发布说明](docs/release-notes-v1.2.9.md)。
