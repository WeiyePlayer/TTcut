...
> 当前稳定版本为 `v1.1.4`，面向 Windows x64；应用不再按 Windows build 或系统版本进行硬拦截。

## 下载与安装

1. 从 [TTcut v1.1.4 Release](https://github.com/WeiyePlayer/TTcut/releases/tag/v1.1.4) 下载 `TTcut-1.1.4-x64-Setup.exe`。
...
- 设置页新增“默认模型”和 CUDA 专用“新模型”两个球识别档位；新模型使用 SegFormer++ B2 主模型和 WASB 辅模型的一致性过滤。
- 新模型沿用动态 ROI，并根据解码后的实际视频分辨率计算主辅输入尺寸；两个权重约 105 MB，仅在首次选择时下载并逐文件校验。
- 修复新模型预处理色彩条件、短缺失轨迹和反弹平台计数，分析结果继续接入现有回合分组与 `bounce_count`。
- 这是独立 Beta 更新通道：现有稳定版用户不会自动收到；安装 Beta 的用户仍会接收后续 Beta 和正式版更新。

下载与完整说明见 [v1.2.0-beta.1 Release](https://github.com/WeiyePlayer/TTcut/releases/tag/v1.2.0-beta.1) 和[发布说明](docs/release-notes-v1.2.0-beta.1.md)。

## v1.1.4 更新

- 修复“视频处理组件”和可选 x264 编码组件的下载链接失效问题，首次安装或重新安装组件可正常下载。
- 新地址固定到 TTcut 运行时资产仓库中已验证的归档；文件名、大小、SHA-256、版本和安装布局均保持不变。
- 已经安装完成的组件不受影响，不需要为了本次更新重新下载。

详见 [v1.1.4 发布说明](docs/release-notes-v1.1.4.md)。
