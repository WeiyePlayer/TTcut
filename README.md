...
> 当前稳定版本为 `v1.1.5`，面向 Windows x64；应用不再按 Windows build 或系统版本进行硬拦截。

## 下载与安装

1. 从 [TTcut v1.1.5 Release](https://github.com/WeiyePlayer/TTcut/releases/tag/v1.1.5) 下载 `TTcut-1.1.5-x64-Setup.exe`。
...
- 修复有效视频导出被误判为“导出时长不匹配”并报告失败的问题。
- 导出结果现在统一使用既有的 2 秒领域容差；偏差不超过 2 秒时接受，超过 2 秒时仍会拒绝。
- 分辨率、编码、音频、时间戳、旋转和输出文件完整性等其他导出校验保持不变。

详见 [v1.1.5 发布说明](docs/release-notes-v1.1.5.md)。
