...
> 当前 Windows x64 与 macOS 15+ Apple Silicon 稳定版本均为 `v1.3.4`。
...
- Windows 与 macOS 自定义剪辑新增“原片播放 / 回合播放”切换，并加入以鼠标位置为中心的时间轴滚轮缩放工具。
- Windows 更新改为先提示再下载，支持稍后提醒、跳过当前版本，以及下载完成后自行选择何时重启安装。
- 修复加载和代理准备阶段播放目标丢失的问题，增强完全停帧时的兼容预览恢复；同时补充 GPU 精度回退、检出统计和本地崩溃诊断。

详见 [v1.3.4 发布说明](docs/release-notes-v1.3.4.md)。
...
- 使用“落台判定”时，板数是弹跳代理值，不是真实击球计数；使用“连续运动”时不显示板数。
