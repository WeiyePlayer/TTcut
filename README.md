...
> 当前稳定版本为 `v1.2.10`，面向 Windows x64。
...
- 设置新增“重编码为固定帧率”选项，默认关闭。关闭时可变帧率视频直接使用原始媒体；开启后会先生成固定帧率处理媒体，以提高剪辑时间点的稳定性，但会增加处理时间和磁盘占用。
- 修复安装程序在部分环境中被预安装磁盘空间探测错误阻断的问题。

详见 [v1.2.10 发布说明](docs/release-notes-v1.2.10.md)。
