...
> 当前稳定版本为 `v1.2.1`，面向 Windows x64；应用不再按 Windows build 或系统版本进行硬拦截。

## 下载与安装

1. 从 [TTcut v1.2.1 Release](https://github.com/WeiyePlayer/TTcut/releases/tag/v1.2.1) 下载 `TTcut-1.2.1-x64-Setup.exe`。
...
- 修复部分视频包含异常帧时长时，分段导出可能出现音画时长偏差并被判定失败的问题。
- OpenH264 与 x264 裁剪路径会清除输入继承的帧率元数据，同时继续保留可变帧率，不强制转换为固定帧率。
- 静音视频的分段拼接清单现在写入所选片段的明确时长，避免异常数据延续到最终输出。
- 音画同步校验失败时，日志会记录视频时长、音频时长、偏差和允许范围，便于定位问题。
- 本版本不修改分析模型、动态 ROI、球轨迹、回合或板数识别逻辑。

详见 [v1.2.1 发布说明](docs/release-notes-v1.2.1.md)。
