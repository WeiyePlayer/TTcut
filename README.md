...
> 当前 Windows 稳定版本为 `v1.3.7`；macOS 15+ Apple Silicon 当前稳定版本为 `v1.3.4`。
...
- 改善硬件加速分析遇到显存或批处理限制时的自动恢复，减少整段分析失败。
- 修复自定义剪辑预览的时间线定位，并让混合回合识别按原视频时间判断片段边界。
- 完整安装包补充所需的系统运行库，并改进运行资源检查提示。

详见 [v1.3.7 发布说明](docs/release-notes-v1.3.7.md)。
