...
> 当前稳定版本为 `v1.1.0`，面向 Windows x64；应用不再按 Windows build 或系统版本进行硬拦截。

## 下载与安装

1. 从 [TTcut v1.1.0 Release](https://github.com/WeiyePlayer/TTcut/releases/tag/v1.1.0) 下载 `TTcut-1.1.0-x64-Setup.exe`。
...
- 分析区域根据每个视频的球桌标定动态计算，只将相关画面送入 TrackNet；默认采用 1.25× ROI 输入档位，并将结果映射回原视频坐标。
- 优化 CUDA 分析流水线与自动分析进度，减少无界缓存和等待，并移除面向测试的中间弹窗。
- 自动标定改用元数据驱动的采样定位，提升长视频抽帧效率和多帧一致性判断。
- 新增批量任务，可一次选择多个 MP4 视频并串行完成自动标定、分析与导出；单项失败不会中断后续任务。
- 安装包内置回合分析和球桌识别双模型，首次使用无需另行下载模型文件。
- 新增应用内更新检查与后台下载，更新就绪后可重启安装。
- 新增可选 `libx264` 编码组件，支持高于 4K 的 H.264 重编码；未安装或校验失败时继续使用 OpenH264。
- 修复分段导出时的 AAC 时间戳处理，降低合并片段出现音画时间轴异常的风险。
- 使用带安装向导的 NSIS 安装器，可选择安装根目录、桌面快捷方式，并迁移旧版安装数据。
- 删除 Windows 系统版本硬检测，改由实际组件和任务自检判断可用性。
...
在“批量任务”中一次选择多个 MP4 视频。任务固定使用自动标定并按列表顺序串行执行，输出保存到各源视频目录；某个视频失败后会记录原因并继续处理其余任务。
...
- 单视频工作流一次处理一个 MP4；批量任务支持一次选择多个 MP4 并串行执行。
- 板数是弹跳代理值，不是真实击球计数。
