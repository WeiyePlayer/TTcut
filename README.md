...
视频、分析结果和历史记录只保存在本机；软件不要求登录、不上传视频、不采集遥测。BlurBall（默认）、TrackNet 和球桌识别模型均随安装包提供，不需要另行下载权重。首次安装分析运行时和视频处理组件需要联网，组件安装完成后可以离线分析、预览和剪辑。

> 当前稳定版本为 `v1.2.4`，面向 Windows x64；应用不再按 Windows build 或系统版本进行硬拦截。

## 下载与安装

1. 从 [TTcut v1.2.4 Release](https://github.com/WeiyePlayer/TTcut/releases/tag/v1.2.4) 下载 `TTcut-1.2.4-x64-Setup.exe`。
...
- 修复长视频在 CPU 导出时可能于关键帧探测阶段固定 120 秒后失败的问题。
- 关键帧发现改用 FFprobe packet-level 探测，避免在分段编码前解码扫描整段视频。
- 导出、输出校验和组件设置子进程不再依赖统一的 120 秒截止时间；耗时任务可以按实际处理时间继续运行。
- 用户取消导出或退出应用时仍会终止当前 FFprobe/FFmpeg 进程树，并返回正确的取消状态。
- 本版本不修改分析模型、动态 ROI、球轨迹、回合识别、板数识别或用户选择的剪辑范围。

详见 [v1.2.4 发布说明](docs/release-notes-v1.2.4.md)。
...
- **自定义**：进入独立时间轴逐项选择回合；每个片段均可预览并独立调整起止边界。

![选择剪辑模式](docs/images/cutting-modes.png)

### 4. 设置剪辑边界并导出

“所有回合”和“精彩回合”沿用“设置”中的回合前后保留时间；“自定义”模式可在独立时间轴中直接调整每个片段的起止边界，然后开始导出。输出保存在原视频目录：
...
安装包内置固定的 BlurBall、TrackNet 和球桌识别模型。按需安装的分析运行时组件包含 Python 3.12.13、PyTorch 2.12.1、NumPy、OpenCV 和最小分析 Worker，负责：

- 默认使用 BlurBall 定位乒乓球，也可在设置中切换为 TrackNet。
- 自动或手动球桌标定与坐标映射。
- 按所选球模型应用对应的弹跳检测，并使用统一的球台区域、时间间隔和回合规则。
- 只基于弹跳事件的回合分组。
