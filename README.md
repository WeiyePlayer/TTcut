...
> 当前稳定版本为 `v1.2.3`，面向 Windows x64；应用不再按 Windows build 或系统版本进行硬拦截。

## 下载与安装

1. 从 [TTcut v1.2.3 Release](https://github.com/WeiyePlayer/TTcut/releases/tag/v1.2.3) 下载 `TTcut-1.2.3-x64-Setup.exe`。
...
- 时间轴支持拖动片段开始和结束边界、拖动播放头定位；普通滚轮平移，按住 `Ctrl` 滚轮可围绕指针位置缩放。
- 支持空格键播放/暂停，视频监视器改为 contain 适配，便于检查完整画面。
- 导出前由 Main 重新校验回合 ID、时间范围、帧长度、排序和重叠关系；非法自定义范围会被拒绝，不会交给 FFmpeg。
- 取消导出后保留本次自定义编辑状态；返回模式页时会重置，避免把旧范围带入下一次编辑。

详见 [v1.2.3 发布说明](docs/release-notes-v1.2.3.md)。
