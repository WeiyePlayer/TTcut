...
> 当前稳定版本为 `v1.2.11`，面向 Windows x64。
...
- 设置新增“回合识别方式”。默认“落台判定”保留既有行为；可选“连续可见”，用于在落台事件不足但球持续可见时整理回合。
- 连续可见方式按完整分析运行，精彩回合按时长档位筛选；各分析结果会保存实际使用的识别方式，历史记录据自身结果显示信息。
- 修复连续可见方式在视频末段、短而明确回合及小球画面中的识别问题，并提升运行资源安装过程的稳定性。

详见 [v1.2.11 发布说明](docs/release-notes-v1.2.11.md)。
...
- **所有回合**：剪辑全部有效回合。
- **精彩回合**：使用“落台判定”时按板数筛选，使用“连续可见”时按时长档位筛选。
...
- 使用“落台判定”时，板数是弹跳代理值，不是真实击球计数；使用“连续可见”时不显示板数。
