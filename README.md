...
> 当前稳定版本为 `v1.2.2`，面向 Windows x64；应用不再按 Windows build 或系统版本进行硬拦截。

## 下载与安装

1. 从 [TTcut v1.2.2 Release](https://github.com/WeiyePlayer/TTcut/releases/tag/v1.2.2) 下载 `TTcut-1.2.2-x64-Setup.exe`。
...
- 修复多片段 AAC 导出因编码时长舍入累积而产生音画偏差的问题；拼接现在使用编码后片段的实测时长，并将音频安全补齐到画面端点。
- x264 分段编码会避免在拼接边界产生回退或相等的解码时间戳，降低 `EXPORT_CONCAT_FAILED` 风险。
- 导出已经生成可播放文件、但后续校验或记录步骤发生异常时，文件会以防冲突名称保留，不再直接删除。
- 单视频和批量流程会把这类结果标记为“已导出，但存在异常”，提供错误代码、技术详情和日志入口，并允许继续处理后续视频。
- 无法读取的输出和用户主动取消仍按失败或取消处理；本版本不修改分析模型、动态 ROI、球轨迹、回合或板数识别逻辑。

详见 [v1.2.2 发布说明](docs/release-notes-v1.2.2.md)。
