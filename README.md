...
> 当前稳定版本为 `v1.1.2`，面向 Windows x64；应用不再按 Windows build 或系统版本进行硬拦截。

## 下载与安装

1. 从 [TTcut v1.1.2 Release](https://github.com/WeiyePlayer/TTcut/releases/tag/v1.1.2) 下载 `TTcut-1.1.2-x64-Setup.exe`。
...
- 新增签名更新清单和固定发布公钥验证，恢复自签名正式包后续版本的安全自动更新；`v1.1.0`、`v1.1.1` 用户需要手动安装本版本一次。
- 单视频和批量任务支持 MP4、MOV；竖屏与带旋转元数据的视频会正确适配标定、预览、历史封面和 MP4 导出。
- 单视频导出完成或批量队列处理完当前可执行项目后显示非阻塞支持提示，可关闭或选择 30 天内不再显示。

详见 [v1.1.2 发布说明](docs/release-notes-v1.1.2.md)。
