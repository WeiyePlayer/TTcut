...
it.each(['darwin', 'win32'] as const)('opens public links on %s', async platform => {
...
it('rejects removed component-download links and other targets', async () => {
  const url = 'https://example.com/component-download';
  await expect(openExternalUrl(url, 'win32')).rejects.toThrow('EXTERNAL_URL_REJECTED');
