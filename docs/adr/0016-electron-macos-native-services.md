...
The first delivery targets macOS 15+ and arm64, bundles one offline runtime, and produces local ad-hoc signed app/DMG/ZIP artifacts. It has no production update feed or automatic installer. The superseded SwiftUI sources are retained only in Git history; current data lives under Electron's Application Support/TTcut-Electron directory. Closing the window hides it; explicitly quitting cancels active work only after the existing confirmation.
