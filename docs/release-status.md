...
- 125%、150%、200% 强制缩放下的 840×520 Electron 布局自动化通过；它只作为 DPI 预检查，不替代干净系统矩阵。
- 从现有 0.1.2 每用户安装升级到 1.0.0 成功，受管 CUDA 运行时和权重文件未被重复下载或改写，安装后窗口可正常启动。
...
- Electron 43.1.1 Windows x64 构建缓存使用官方 Release SHA-256 固定校验；重建日志已明确显示命中本地缓存，不再请求临时下载校验文件。
