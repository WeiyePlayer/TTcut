...
`v1.3.5` is a stable update for Windows x64 and macOS 15+ on Apple Silicon. Both platforms gain bounce-count multi-selection and improved shared-boundary timeline interaction. Windows also reduces BlurBall analysis time with constrained temporal sampling.
...
- Continuous-motion results now carry separate bounce-detection metadata. Windows custom cutting reuses existing BlurBall landing results. Native macOS Core ML continuous-motion analysis adds trajectory-change bounce detection without changing existing rally boundaries, enabling bounce-count display and filtering.
...
## Windows BlurBall analysis throughput
...
- **macOS 15+ · Apple Silicon · v1.3.5**: `TTcut-1.3.5-arm64-Setup.dmg` (recommended) and `TTcut-1.3.5-arm64-Setup.zip`, with native Core ML analysis and media runtimes included. Intel Macs are not supported.
- The macOS packages are ad-hoc signed and not notarized. If first launch is blocked, allow the app in System Settings > Privacy & Security. In-app updating remains unavailable. The Windows temporal-sampling strategy is not enabled on macOS, and the RTX benchmark is not a macOS performance claim.

## Verification

- Windows release verification: TypeScript type checking passed. Vitest passed 431 tests across 63 files, with 25 tests across 6 files skipped by their existing conditions. All 256 Python regression tests passed.
...
- macOS passed TypeScript, 102 relevant Vitest tests, 17 macOS-specific tests, 27 native Swift tests, Core ML resource verification, and production Electron/packaged UI acceptance. DMG/ZIP passed arm64 bundle, signature, dependency, and delivery checks.
