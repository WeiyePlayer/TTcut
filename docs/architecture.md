...
原始 MP4/MOV -> ffprobe -> 手动四点或自动五帧球台标定（始终原始媒体）
             -> VFR 判断 -> 设置关闭：原始 VFR；设置开启：CFR 派生媒体缓存（失败则显式 VFR 回退）
             ...
             -> FFmpeg（CFR 保持目标帧率，原始 VFR/回退沿用 VFR 策略）
             -> .partial.mp4 -> 探测/同步/元数据验证
             -> 原子改名 -> ttcut-media:// 成片预览
```

原始媒体负责身份和标定；处理媒体负责球路分析、分析后预览、剪辑和新分析的 XML。设置“重编码为固定帧率”默认关闭：VFR 分析直接使用原始媒体，不创建缓存也不显示回退警告；启用后才进入 CFR 缓存和验证路径。旧历史记录没有 `processing` 来源块时保持旧的 VFR 语义。CFR 缓存使用任务隔离的 partial 文件、原子提交和完整校验；失败/取消的本次新缓存会被删除，删除最后一个引用时再清理已成功缓存。
