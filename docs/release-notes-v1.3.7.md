...
- macOS 发布验证：类型检查、17 项 macOS 边界测试、192 项预览/契约/回合边界相关测试和启用原生依赖的 Swift 测试通过；Swift Core 专项共 19 项通过。
- macOS 打包应用通过 21 项真实 Electron 验收和 3 项批处理界面验收，覆盖 Core ML、120 fps 源时间结果、full-range 兼容预览、HDR/VFR、原生导出及排除区间防误合并。
...
- 本次未进行 Windows 真实视频效果对照或安装器的人工安装测试；macOS 120 fps 推理验收使用合成视频，不作为真实比赛原片准确率证据。
