...
BlurBall uses FP16 Core ML with CPU/Neural Engine, four asynchronous predictions and four prefetched windows. Results are consumed in frame order. Model input/output arrays remain Float32. The table model uses FP32 Core ML on the CPU; its GPU token matching exceeded the fixed numeric tolerance on the verification host. The original architecture and checkpoint weights are retained. FP16 is an accepted precision tradeoff, not numerically identical to FP32; see [the adoption record](../docs/performance/macos-fp16-adoption-2026-09-05.md). Python is only the development reference.
