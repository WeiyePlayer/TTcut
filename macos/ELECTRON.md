...
Forge packages the application. `make-macos.mjs` signs the assembled Electron app and invokes the existing electron-builder v26 with a separate Mac configuration and publishing disabled. Output is `out/TTcut-darwin-arm64/TTcut.app`, `TTcut-${version}-arm64-Setup.dmg`, and `TTcut-${version}-arm64-Setup.zip` under `out/make/macos/arm64/`. The release manifest excludes updater metadata because macOS in-app updates are disabled. `--skip-native` reuses a staged runtime; use only when native source/assets have not changed. `--app-only` omits DMG/ZIP generation.
