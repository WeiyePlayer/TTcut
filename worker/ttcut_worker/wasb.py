...
class HRNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for scale in self._out_scales:
            ...
            y_out[scale] = y
        return y_out, features_before_deconv
...
class WASBNet(nn.Module):
    def __init__(self, in_frames, resolution, pretraining=False, classify_invisible=False):
        super(WASBNet, self).__init__()
        W, H = resolution
        config = {
            "name": "hrnet",
            f"frames_in": 1,
        ...
        self.model = HRNet(cfg=config)
        self.classify_invisible = classify_invisible
        if classify_invisible:
            self.cls_layer = torch.nn.Linear(in_features=self.model.stage4_cfg["NUM_CHANNELS"][-1], out_features=2)

        # Adjust number of input channels
        if in_frames != 1:
            conv1 = nn.Conv2d(3 * in_frames, self.model._stem_inplanes, kernel_size=3, stride=self.model._stem_strides[0], padding=1, bias=False)
            # Init weights of new channels
            with torch.no_grad():
                conv1.weight = torch.nn.Parameter(torch.cat([self.model.conv1.weight for _ in range(in_frames)], dim=1) / in_frames)
            self.model.conv1 = conv1
        if pretraining:
            raise ValueError("Offline inference does not load WASB pretraining assets.")


    def forward(self, x):
        # three_frame_out = self.model(x)[0]
        out, features = self.model(x)
        three_frame_out = out[0]
        if self.classify_invisible:
            ...
            class_out = self.cls_layer(pooled)
            return three_frame_out[:, 1:2], class_out # return the middle frame
        else:
            return three_frame_out[:, 1:2], None
