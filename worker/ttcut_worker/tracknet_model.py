...
from .errors import DeviceError, ModelResourceError, WeightError, WorkerError
...
def create_tracknet(seq_len: int, bg_mode: str):
    ...
    class TrackNet(nn.Module):
        ...
        def forward(self, value):
            x1 = self.down_block_1(value)
            value = nn.MaxPool2d(2, 2)(x1)
            x2 = self.down_block_2(value)
            value = nn.MaxPool2d(2, 2)(x2)
            x3 = self.down_block_3(value)
            value = nn.MaxPool2d(2, 2)(x3)
            value = self.bottleneck(value)
            value = self.up_block_1(torch.cat([nn.Upsample(scale_factor=2)(value), x3], dim=1))
            value = self.up_block_2(torch.cat([nn.Upsample(scale_factor=2)(value), x2], dim=1))
            value = self.up_block_3(torch.cat([nn.Upsample(scale_factor=2)(value), x1], dim=1))
            return self.sigmoid(self.predictor(value))
...
def load_tracknet(weight_value: str | Path, requested_device: str) -> LoadedTrackNet:
    ...
    if not path.is_file():
        raise ModelResourceError(f"Local TrackNet test weight is missing: {path}")
    torch = import_torch()
    try:
        ...
        raise ModelResourceError(f"Local TrackNet test weight is invalid: {path}") from exc
